...
3. **Offline stub tests** (run keyless in CI): assert the exact request path/headers/payload your adapter sends, response parsing, and error mapping (bad status → `Error::Provider` naming the fix). **For any chat provider: 'success is not a summary' is a REQUIRED assertion** — an HTTP 200 with empty/whitespace content (e.g. a reasoning model exhausting max_tokens in thinking) must be a NAMED failure with the fix, never an empty Summary (DL-014).
