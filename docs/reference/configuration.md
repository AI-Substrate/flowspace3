...
- Unknown **top-level sections in the file** are ignored so an older binary can
  still read machine-wide configuration written for a newer version. They
  configure nothing; `flowspace3 config show` and `flowspace3 doctor` name every
  ignored section. This kills the cross-version flag-day without making the
  ignored content silent.
- Unknown **keys inside a known file section** remain a startup failure. That is
  the likelier and more dangerous typo: `[embedder] activ = "big"` must not
  silently leave the default provider active while looking configured.
- Environment overrides are strict at both levels. `FS3_DATABSE__URL` and
  `FS3_EMBEDDER__ACTIV` both fail by variable name: unlike a shared file written
  by another binary version, an unknown override has no forward-compatible
  author and is a current human mistake.
...
| `worktree_reconcile_ticks` | integer | `6` | Shared five-second reconcile ticks between linked-worktree scans; `6` is a 30-second cadence and `0` disables discovery. Thirty seconds stays below the normal create-then-query gap without spawning one `git worktree list` per registered repo every tick — about 52k subprocesses/day at three repos for an event that happens weekly. | `FS3_INDEXING__WORKTREE_RECONCILE_TICKS` |
| `job_retention_days` | integer | `1` | Days to retain completed jobs before the daemon purges them in bounded batches. Must be at least 1. The one-day default follows measured churn of at least 515k completed rows/day; seven days would retain roughly 3.6M rows. | `FS3_INDEXING__JOB_RETENTION_DAYS` |
| `conversation_poll_ticks` | integer | `12` | Shared five-second reconcile ticks between native Claude/OMP conversation polls; `12` is one minute and `0` disables polling. Each pass submits at most 10 parent sessions with up to six-second staggering, shortened to cadence/10 for faster polls. Status and doctor report daemon-owned flowing/stalled/disabled state. | `FS3_INDEXING__CONVERSATION_POLL_TICKS` |
| `conversation_lookback_days` | integer | `14` | Ignore cursorless native session files older than this window. Must be at least 1; already-tracked files remain eligible regardless of age, including growth, truncation, and detectable replacement. | `FS3_INDEXING__CONVERSATION_LOOKBACK_DAYS` |
