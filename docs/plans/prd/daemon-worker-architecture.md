...
- **Periodic full-walk backstop**: still the highest-value follow-up, though the watcher's `record_walk` shrank its job. It remains the answer to directories removed wholesale, to directories still inside their debounce window when the daemon dies, to an edit landing between `add` and the next reconcile pass, and to the inotify recursive-emulation race in general.
- **Per-path upsert verb in `fs3-store`**: `sync_worktree_files` only takes the WHOLE map, so the watcher writes N rows (the whole worktree's file count — 133 on this repo, ~10k on a monorepo) to record the handful it walked. A `touch_worktree_file(worktree, path, blob)` would write one. Rejected for the watcher landing on two grounds, both recorded deliberately: it is a cross-crate change outside that worker's fence, and it does NOT reap deletions, which the whole-map write does. Profile-triggered — revisit when the write shows up on a large repository.
