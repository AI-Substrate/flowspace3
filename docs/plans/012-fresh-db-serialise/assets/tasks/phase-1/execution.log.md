...
## Follow-up 012b — close review nits and ship proof

- Clamped legacy seed timestamp decoding to 2020-01-01 through current time plus one hour. A current `unique_seed` round-trips within one second; pre-2020 and beyond-tolerance seeds are rejected. Green: `artifact://146`; removing the clamp failed on the lower bound (`artifact://136`).
- Moved test-hook construction out of the shipped `create_database` body into `CreateDatabasePermit`, preserving guard drop order and both observable create-path tests. Green: `artifact://144`; mutating the semaphore to eight permits failed both tests at eight concurrent creates (`artifact://140`).
- Shipped the attributed DDL instrument as executable `bin/ac-0001-ddl-probe.sh`, byte-identical to the governance source except its usage path, and pointed ac-0001's receipt at it. `--check` ran no work; the real `pg_first_light` run reported `exit=0 samples=173 max_concurrent_ddl=1 samples_with_ddl=104 samples_over_1=0 foreign_ddl_max=0`.
- Committed the review record at MD5 `12c2eba781e4ad71ab3bd4c46ac84b95` (JSON) and `e88a596ae8e5296f629058cc7e59f894` (Markdown); `ddocs validate` returned zero errors/warnings.
- Active plan, implementation-guide, task, and backpressure text now targets the landed `flowspace3-db-test` postmaster on :5434 and records delta verdict APPROVE with findings closed or explicitly accepted. Historical :5433 measurements remain unchanged in their receipts and review record.
