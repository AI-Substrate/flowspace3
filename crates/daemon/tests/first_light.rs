...
/// The registry claim: two repositories can name different instances, and the
/// rows they produce are keyed by what ANSWERED, not by what config asked for.
#[tokio::test]
async fn two_repos_on_different_instances_write_different_model_keys() {
    let stack = Stack::create("perrepo").await;

    // The default instance answers for an unknown repo; a repo naming another
    // instance gets that one. Both are fakes here, which is the point: the
    // resolution is what is under test, not the provider.
    let default_key = stack.state.embedder_key("");
    let unknown_repo_key = stack.state.embedder_key("git:example.com/never-configured");
    assert_eq!(
        default_key, unknown_repo_key,
        "an unknown repo falls back to the active selection rather than failing"
    );
    assert!(
        !default_key.is_empty(),
        "the key comes from provider.key() and is never blank"
    );
    assert!(
        default_key.contains(&fs3_store::EMBEDDING_DIMENSIONS.to_string()),
        "the embedder key carries the WIDTH, so two spaces can never be compared: {default_key}"
    );
